...
pub use monthly::{
    DailyAnomaly, MonthlyReport, format_minutes, format_signed_minutes, generate_monthly_report,
    maybe_send_monthly_summary, parse_month, previous_month, summarize_month,
};
...
pub fn select_wake_event(log: &str, date: NaiveDate, search: &SearchConfig) -> Option<NaiveTime> {
    display_event_times(log, date, "Display is turned on")
...
pub fn select_display_off_event(
    ...
    display_event_times(log, date, "Display is turned off")
        .filter(|time| *time >= start && *time <= accounting.reasonable_end_end)
        .max()
}

fn display_event_times<'a>(
    log: &'a str,
    date: NaiveDate,
    event: &'static str,
) -> impl Iterator<Item = NaiveTime> + 'a {
    let date_prefix = date.format("%Y-%m-%d").to_string();
    log.lines()
        .filter(move |line| line.starts_with(&date_prefix) && line.contains(event))
